...
class MultiNamespaceIT {
    ...
    void testCrossNamespaceNetworking() {
        // This test demonstrates how you might test cross-namespace connectivity
        // All three namespaces (frontend, backend, monitoring) are available

        // Create a service in backend namespace
        Service apiService = new ServiceBuilder()
            .withNewMetadata()
            .withName("api-service")
            .withNamespace("backend")
            .endMetadata()
            .withNewSpec()
            .withSelector(Map.of("app", "api"))
            .addNewPort()
            .withPort(3000)
            .withTargetPort(new IntOrStringBuilder().withValue(3000).build())
            .endPort()
            .endSpec()
            .build();

        resourceManager.createResourceWithWait(apiService);
